...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1,
        ...
        with tf.Session() as sess:
            ...
            if restore:
                #restore_path = os.path.join(output_path, "savefile_k")
                ckpt = tf.train.get_checkpoint_state(output_path)#改动为恢复的模型路径
            ...
            for epoch in range(epochs):
                ...
                for step in range((epoch * training_iters), ((epoch + 1) * training_iters)):
                    ...
                    if step % display_step == 0:
                        self.output_minibatch_stats(sess, summary_writer, step, batch_x,
                                                    util.crop_to_shape(batch_y, pred_shape), epoch, training_iters)
                ...
                #每5个epoch一次保存模型
                if (epoch%5 == 0):
                    save_path = tf.train.Saver.save(sess, save_path+"speech", global_step = epoch)
    ...
    def output_epoch_stats(self, epoch, total_loss, training_iters, lr):
        ...
        filename = '/Users/imke/Downloads/tf_unet/epoch'+str(epoch)+'.txt'
        with open (filename, 'a') as fileobj:
            fileobj.write("Epoch {:}, Average loss: {:.4f}, learning rate: {:.4f}\n".format(epoch, (total_loss / training_iters), lr))

    def output_minibatch_stats(self, sess, summary_writer, step, batch_x, batch_y, epoch, training_iters):
        ...
        filename = '/Users/imke/Downloads/tf_unet/epoch'+str(epoch)+'.txt'
        with open (filename, 'a') as fileobj:
            fileobj.write(str(step)+"\t"+str(loss)+"\t"+str(acc)+"\t"+str(error_rate(predictions, batch_y))+"\n")
...
def error_rate(predictions, labels):
    ...
    These are two 4D Matrixes both predictions and labels
    """
